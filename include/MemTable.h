...
public:
    ...
    MemTable(const std::string &dir, FILE *log, bool snap = false)
        : log(log), dir(dir)
    {
        if (snap)
        {
               ...
               std::unordered_set<std::string> &used_key) const
    {
        ...
        while (node)
        {
            if ((lb || lower <= node->keypair.key) &&
                (ub || upper >= node->keypair.key))
            {
                ...
                std::ignore =
                    fread((void *) val.data(), 1, node->keypair.len, file);
    ...
    bool put_clone(const KeyOffset &keypairs)
    {
        ...
        {  //插入新的头结点，加层
            ...
            head = new Node();
            head->right = new Node(
                nullptr, down_node, keypairs.key, offset, keypairs.len);
            head->down = oldHead;
        }
        entry_size++;
